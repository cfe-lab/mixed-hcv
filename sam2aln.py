...
READ_MERGE_BUFFER_SIZE = READ_MERGE_CHUNK_SIZE * nthreads * 10
...
def matchmaker(samfile, semaphore=None):
    ...
    :param semaphore: acquired before each pair is generated, unless it is None
...
def sam2aln(remap_csv, aligned_csv, insert_csv, failed_csv, nthreads=None):
    ...
    if nthreads:
        semaphore = Semaphore(READ_MERGE_BUFFER_SIZE)
        iter = parse_sam_in_threads(remap_csv, nthreads, semaphore)
    else:
        semaphore = None
