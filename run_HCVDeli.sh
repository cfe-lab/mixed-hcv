...
for run in ${runs[@]}; do
    ...
    for quality in ${qualities[@]}; do
        for minwidth in ${minwidths[@]}; do
            ...
            python collectcat.py -runname ${run} -output /media/macdatafile/mixed-hcv/staging/ -x ${ref} -minq q${quality} -min_target_width ${minwidth} --cache /media/macdatafile/mixed-hcv/cache/ ./out/$run/${run}__deli__${refnice}__q${quality}__mw${minwidth}.csv.*
